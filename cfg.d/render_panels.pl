...
$c->{render_panels_pagination_min} = 5; # add pagination controls if there are at least this number of panels
$c->{render_panels} = sub
{
  ...
  my $buttons = $repository->make_element( "ul", class => "ep_panel_buttons", id => "${id}_buttons", role => "tablist", "aria-label" => "Tabbed Panels" );
  ...
  foreach my $p ( @${panels_to_show} )
  {
    ...
    if( $has_content || $panels->{$p}->{data}->{panel_frag} || $show_empty )
    {
      ...
      if( $use_panel )
      {
        ...
        my $button = $repository->make_element( "li", "class" => "ep_panel_links ${id}_links", id => "${id}_links_$p", onfocusin => "ep_open_panel(event, '$id', '$p')", "tabindex" => -100-$number_of_panels, role => "tab", "aria-controls" => "$p" );
  ...
  if( $first ) # anything to show?
  {
    ...
    my $toggle_a = $repository->make_element( "a", id => "${id}_controls_toggle_a", class => "ep_panel_controls_toggle_a", onclick => "ep_toggle_a_panel('$id');" );
    $toggle_a->appendChild( $repository->make_text( "[A]" ) );
    $controls->appendChild( $toggle_a );
    my $toggle_b = $repository->make_element( "a", id => "${id}_controls_toggle_b", class => "ep_panel_controls_toggle_b", onclick => "ep_toggle_b_panel('$id');" );
    $toggle_b->appendChild( $repository->make_text( "[B]" ) );
    $controls->appendChild( $toggle_b );
